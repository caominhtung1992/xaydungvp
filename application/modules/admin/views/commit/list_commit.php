<div class="paddings">
  <div id="action-links">
    <ul>
      <li id="add-prod"><a href="<?php echo base_url()."admin/commit/add"; ?>">Thêm </a></li>
    </ul>
  </div>
<style type="text/css">
      ...
      <?php
	  	if(isset($list_sup) && $list_sup!= NULL){
			...
			foreach($list_sup as $value){
				$stt++;
	  ?>
      <tr id="row_<?php echo $value['commit_id']; ?>" onMouseOver="this.className='row-hover'" onMouseOut="this.className=''" class="">
        <td width="50px" style="text-align:center;"><?php echo $stt; ?></td>
        <td width="222"><?php echo $value['commit_name']; ?></td>
        <td width="60px"><a href="<?php echo base_url()."admin/commit/update/".$value['commit_id'].""; ?>">Sửa lại</a></td>
        <?php echo "<td width='40px'><a href='javascript:del_cago(".$value['commit_id'].")'>Xóa</a></td>"; ?>
      </tr>
      <?php
		}; }else{
...
<script type="text/javascript">
	$(document).ready(function(e) {
        $(".status_active").live("click",function(){
			...
			$.ajax({
				"url"	: "<?php echo base_url()."admin/commit/update_status"; ?>",
				"type"	: "post",
		...
		$(".change_order").change(function(){
			...
			type = "bachnx";
			$.ajax({
				"url" 	: "<?php echo base_url()."admin/commit/update_order"; ?>",
				"type"	: "post",
	...
	function del_cago(id) {
		if(confirm("Bạn chắc chắn muốn xóa bỏ danh mục này ?")){
			$.post("<?php echo base_url()."admin/commit/del"; ?>", {action: "del_cate", id : id}, function(data) { $("#row_"+id).fadeOut();
			} );
		}
	}
</script>